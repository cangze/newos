...
/**
 * An implementation of condition variables built upon semaphores(信号量).
 *
 * <p>
 * A condition variable is a synchronization primitive（同步原语） that does not have
 ...
 * until woken; then reacquire（再次索取） the lock.
 *
 ...
 *
 * <p>
 * Every condition variable is associated with some lock. Multiple（很多） condition
 * variables may be associated with the same lock. All three condition variable
 * operations（所有的三个条件变量） can only be used while holding the associated lock.
 *
 * <p>
