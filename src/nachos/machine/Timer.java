 ...
 * clock ticks. This means that it can be used for implementing time-slicing（时钟切片）,
 * or for having a thread go to sleep for a specific period of time.
 *
 * The <tt>Timer</tt> class emulates（仿真） a hardware timer by scheduling a timer
 * interrupt to occur every time approximately 500 clock ticks pass. There is
 * a small degree of randomness（随机性） here, so interrupts do not occur exactly every
 * 500 ticks.
 */
